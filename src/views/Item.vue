<template>
  ...
  <div class="row">
    ...
    >
      <template #empty> No hi ha articles. </template>
      <template #loading> Carregant articles, si us plau espereu. </template>
      ...
      <Column field="forPurchase" header="Compra" :sortable="true">
        <template #body="slotProps">
          <Checkbox
            v-model="slotProps.data.forPurchase"
            :binary="true"
            :disabled="true"
          />
        </template>
      ...
      <Column field="forManufacture" header="Producció" :sortable="true">
        <template #body="slotProps">
          <Checkbox
            v-model="slotProps.data.forManufacture"
            :binary="true"
            :disabled="true"
          />
        </template>
      ...
      <Column field="forSale" header="Venta" :sortable="true">
        <template #body="slotProps">
          <Checkbox
            v-model="slotProps.data.forSale"
            :binary="true"
            :disabled="true"
          />
        </template>
  ...
  <Dialog header="Article" v-model:visible="display">
    <div>
      <div class="grid p-fluid">
        ...
        <div class="col-12 md:col-4">
          <div class="p-inputgroup">
            <span class="p-inputgroup-addon"> Categoria: </span>
            <Dropdown
  ...
  methods: {
    ...
    openDialog(currentid: number) {
      if (currentid !== 0) {
        ...
        this.currentItem = this.items.find(({ id }) => id === currentid) as any;
        this.display = true;
      } else {
        this.resetData();
        this.fetchData();
    ...
    save() {
      ...
      if (this.currentItem.id === 0) {
        ...
        itemService.update(this.currentItem).then((response) => {
          if (response.status === null) {
            this.$toast.add({
              severity: "error",
              summary: "Error Message",
    ...
    drop(id: number) {
      this.confirm.require({
        message:
          "Vols esborrar el registre seleccionat?" + this.currentItem.code,
        header: "Confirmation",
