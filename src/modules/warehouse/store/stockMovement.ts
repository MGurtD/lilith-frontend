...
export const useStockMovementStore = defineStore({
    ...
    actions: {
        async Create(createRequest: StockMovement){
            ...
            const created = StockMovementService.StockMovementService.Create(createRequest)
            return created;
        },
        async GetBetweenDates(startTime: string, endTime: string){
            this.stockMovements = await StockMovementService.StockMovementService.GetBetweenDates(startTime, endTime);
        },
    },
});