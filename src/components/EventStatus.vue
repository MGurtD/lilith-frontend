<template>
    ...
    <div class="row">
        <DataTable :value="eventStatuses" v-model:selection="currentEventStatus"
        datakey="id" stripedRows responsiveLayouts="scroll" :paginator="true"
        showGridlines :rows="10" @row-click="onRowSelect" >
        <template #empty>
            No hi ha estats.
        </template>
        <template #loading>
            Carregant estats, si us plau espereu.
        </template>  
            ...
            <div class="grid p-fluid">
                ...
                <div class="col-12 md:col-8">
                    <div class="p-inputgroup">
                        ...
                        <InputText placeholder="Descripció" v-model="currentEventStatus.description" />
                    </div>
                </div>
...
export default {
    ...
    methods:{
        ...
        openDialog(currentid){
            if (currentid !== 0){
                this.currentEventStatus = this.eventStatuses.find(({id}) => id === currentid)
                this.display = true
            }else{
                this.resetData()
                this.fetchData()
        ...
        save(){
            if (this.currentEventStatus.id === 0) {
                eventstatusService.set(this.currentEventStatus)
                .then(response => {
                ...
                eventstatusService.update(this.currentEventStatus)
                .then(response => {
                    if (response.status === null){
        ...
        drop(currentid){       
            ...
            this.$confirm.require({
                message: 'Vols esborrar el registre seleccionat?' + this.currentEventStatus.code,
                header: 'Confirmation',
                icon: 'pi pi-exclamation-triangle',
                accept: () => {
                    eventstatusService.delete(currentid)
                    .then(response => {
